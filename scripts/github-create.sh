...
echo "Will create a new *$PRIVACYWORD* repo named mailhog-mail-catcher"
echo "on github.com in:"
echo "    user account: admindaspanel"
echo "    organization: daspanel"
echo "with this description:"
echo Docker image using Mailhog that provides simple SMTP server which catches any message sent to it to display in a web interface.
echo ""
...
curl -u admindaspanel https://api.github.com/orgs/daspanel/repos -d \
    "{\"name\": \"mailhog-mail-catcher\", \"description\": \"Docker image using Mailhog that provides simple SMTP server which catches any message sent to it to display in a web interface.\", \"private\": $PRIVATE_TF}"
